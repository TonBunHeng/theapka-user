...
export function GuestsPage() {
  ...
  return (
    <div className="space-y-6 font-ui">
      {/* Header */}
      <PageHeader
        title={t('guests.title', 'Guest List')}
        subtitle={t('guests.totalCount', { count: guests.length })}
        actions={
                ...
                {filteredGuests.map((guest) => {
                  ...
                  return (
                    <TableRow key={guest.id}>
                      <TableCell>
                        <div className="font-bold text-slate-900 font-ui">{guest.name}</div>
                        {guest.phone && (
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-brand-emerald-700 rounded hover:bg-slate-100"
                ...
                <Card key={guest.id}>
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <h4 className="font-bold text-base text-slate-900 font-ui">
                          {guest.name}
                        </h4>
                        {guest.phone && (
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      <button
                        ...
                        className={`text-xs font-semibold px-2.5 py-1 rounded ${
                          guest.sent_at ? 'text-green-700 bg-green-50' : 'text-slate-500 bg-slate-100'
                        }`}
                      >
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => copyPersonalLink(guest.token)}
                          className="p-2 text-slate-500 hover:text-brand-emerald-700 rounded touch-target"
                          title="Copy Link"
                        >
                          <Copy className="w-4 h-4" />
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() => openEditGuest(guest)}
                          className="p-2 text-slate-500 hover:text-slate-900 rounded touch-target"
                          title="Edit"
                        >
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button
                          ...
                          className="p-2 text-slate-400 hover:text-red-600 rounded touch-target"
                          title="Delete"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
      ...
      >
        <div className="space-y-4 font-ui">
          ...
          <div className="p-4 border-2 border-dashed border-slate-300 rounded bg-slate-50 text-center">
            <input
              type="file"
              accept=".csv"
              onChange={handleCsvFileChange}
              className="text-xs file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-xs file:font-semibold file:bg-brand-emerald-700 file:text-white hover:file:bg-brand-emerald-800 cursor-pointer"
            />
          </div>
          ...
          {csvPreviewRows.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="max-h-60 overflow-y-auto rounded border border-slate-200 text-xs">
